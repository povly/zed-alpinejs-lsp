...
- [ ] **Наблюдаемость ошибок** — заменить молчаливые `catch { /* skip */ }` на `connection.console.warn` с контекстом; метрики производительности индексации
...
- [ ] **Semantic Tokens** — цветовое выделение директив (`x-*`), magic properties (`$*`), модификаторов (`.prevent`) отдельно от HTML-атрибутов
